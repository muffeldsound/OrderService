...
namespace OrderService.Tests
{
    ...
    public class OrderTests
    {
        ...
        public void can_generate_receipt_for_motor_super()
        {
            ...
            var actual = order.GenerateReceipt(FileFormat.Text);
            var expected = 
        ...
        public void can_generate_html_receipt_for_motor_super()
        {
            var order = new Order("Test Company");
            order.AddLine(new OrderLine(MotorSuper, 1));
            var actual = order.GenerateReceipt(FileFormat.Html);
        ...
        public void can_generate_json_receipt_for_motor_kasko()
        {
            var order = new Order("Test Company");
            order.AddLine(new OrderLine(MotorKasko, 1));
            var actual = order.GenerateReceipt(FileFormat.Json);
        
            var expected =
                "{    order-receipt-for: 'Test Company',{quantity:1,product-type: Car Insurance,product:{name: Kasko, price: kr 1 000,00}}    subtotal: kr 1 000,00,    mva: kr 250,00,    total: kr 1 250,00}";
